class ZCL_CUBI_WRKLIST_MATLWH definition
  public
...
CLASS ZCL_CUBI_WRKLIST_MATLWH IMPLEMENTATION.
  ...
  METHOD zif_cubi_wrklst~query_headers.
    ...
    DATA: lv_warehouse_number   TYPE /scwm/lgnum,
          lv_scu_guid           TYPE /scmb/mdl_scuguid,
          lt_mat_id             TYPE STANDARD TABLE OF /sapapo/matid,
          lt_matid_matnr        TYPE STANDARD TABLE OF ltyp_matid_matnr,
          ls_header             TYPE zcubi_worklist_header,
    ...
    FIELD-SYMBOLS: <lfs_select_option> TYPE /iwbep/s_cod_select_option,
                   <lfs_matid_matnr>   TYPE ltyp_matid_matnr.
    ...
    ELSE.
      " Read the warehouse number.
      READ TABLE it_warehouse_select_options ASSIGNING <lfs_select_option> INDEX 1.
      MOVE <lfs_select_option>-low TO lv_warehouse_number.
    ENDIF.
    ...
    SELECT SINGLE scuguid FROM /scwm/t300_md
      INTO lv_scu_guid
      WHERE lgnum EQ lv_warehouse_number.
    ...
    IF lines( lt_mat_id ) GT 0.
      ...
      LOOP AT lt_matid_matnr ASSIGNING <lfs_matid_matnr>.
        ...
        IF lv_is_measured NE abap_true.
          ls_header-warehouse = lv_warehouse_number.
          ls_header-product = <lfs_matid_matnr>-matnr.

          " Select the product description.
          SELECT SINGLE maktx FROM /sapapo/mattxt
            INTO ls_header-product_description
          ...
          CALL BADI lo_badi_handler->get_image_path
            EXPORTING
              iv_product    = ls_header-product
            IMPORTING
              ev_image_path = ls_header-product_image.
  ...
  METHOD zif_cubi_wrklst~query_items.
    ...
    DATA: lv_warehouse_number   TYPE /scwm/lgnum,
          ...
          lt_mat_id             TYPE STANDARD TABLE OF /sapapo/matid,
          lt_matid_matnr        TYPE STANDARD TABLE OF ltyp_matid_matnr,
          ls_item               TYPE zcubi_worklist_item,
          lt_taba               TYPE STANDARD TABLE OF dd07v,
          lt_tabb               TYPE STANDARD TABLE OF dd07v,
          lt_worklist_entries   TYPE HASHED TABLE OF zcubi_worklist_item WITH UNIQUE KEY warehouse product uom,
    ...
    FIELD-SYMBOLS: <lfs_select_option>   TYPE /iwbep/s_cod_select_option,
                   <lfs_matid_matnr>     TYPE ltyp_matid_matnr,
                   <lfs_dom_value>       TYPE dd07v,
                   <lfs_worklist_entry>  TYPE zcubi_worklist_item,
    ...
    " Get the UOM select option values.
    IF it_uom_select_options[] IS INITIAL.
      lt_uom_select_options = me->get_supported_uoms( ).
    ELSE.
      lt_uom_select_options[] = it_uom_select_options[].
    ENDIF.
    ...
    " Grab the MATID of each product for the warehouse.
    SELECT DISTINCT matid FROM /sapapo/matlwh
      INTO TABLE lt_mat_id
      WHERE scuguid EQ lv_scu_guid.

    " If we found some records then grab the matnr and uom's for each matid as well.
    IF lines( lt_mat_id ) GT 0.
      SELECT mk~matid mk~matnr m~meinh FROM /sapapo/matkey AS mk
        INNER JOIN /sapapo/marm AS m ON mk~matid = m~matid
        INTO TABLE lt_matid_matnr
        FOR ALL ENTRIES IN lt_mat_id
        WHERE mk~matid EQ lt_mat_id-table_line
      ...
      " Select all records that are currently in the worklist so we can set the appropriate
      " status.
      SELECT * FROM zcubi_worklist
        INTO CORRESPONDING FIELDS OF TABLE lt_worklist_entries
        FOR ALL ENTRIES IN lt_matid_matnr
        WHERE warehouse EQ lv_warehouse_number
          AND product EQ lt_matid_matnr-matnr.
      ...
      SELECT * FROM zcubi_wrklist_st
        ...
        FOR ALL ENTRIES IN lt_matid_matnr
        WHERE product EQ lt_matid_matnr-matnr.

      LOOP AT lt_matid_matnr ASSIGNING <lfs_matid_matnr>.
        ls_item-warehouse = lv_warehouse_number.
        ls_item-product = <lfs_matid_matnr>-matnr.
        ls_item-uom = <lfs_matid_matnr>-uom.

*        CALL FUNCTION 'CONVERSION_EXIT_CUNIT_OUTPUT'
*          EXPORTING
*            input          = <lfs_matid_matnr>-uom
*          IMPORTING
*            output         = ls_item-uom
*          EXCEPTIONS
*            unit_not_found = 1
*            OTHERS         = 2.
        ...
        " Check if the item exists in the worklist table.  If it does, then
        " set the status to the appropriate value.  Otherwise, set it to "New Item (N)"
        READ TABLE lt_worklist_entries ASSIGNING <lfs_worklist_entry>
          WITH KEY warehouse = lv_warehouse_number product = <lfs_matid_matnr>-matnr uom = <lfs_matid_matnr>-uom.
        IF <lfs_worklist_entry> IS ASSIGNED.
          MOVE-CORRESPONDING <lfs_worklist_entry> TO ls_item.
        ELSE.
          ls_item-status = 'N'.
        ENDIF.
        UNASSIGN <lfs_worklist_entry>.
        ...
        READ TABLE lt_worklist_status ASSIGNING <lfs_worklist_status>
          WITH KEY product = <lfs_matid_matnr>-matnr uom = <lfs_matid_matnr>-uom.
        IF <lfs_worklist_status> IS ASSIGNED.
          MOVE-CORRESPONDING <lfs_worklist_status> TO ls_item.
        ENDIF.
        UNASSIGN <lfs_worklist_status>.

        READ TABLE lt_taba ASSIGNING <lfs_dom_value> WITH KEY domvalue_l = ls_item-status.
        IF <lfs_dom_value> IS ASSIGNED.
          ls_item-status_text = <lfs_dom_value>-ddtext.
        ENDIF.
        UNASSIGN <lfs_dom_value>.

        APPEND ls_item TO lt_items.
      ENDLOOP.
    ENDIF.

    " Determine whether we need to filter based on status or not now.  If so,
    " loop again based on the select options.
    IF lines( it_status_select_options ) GT 0.
      LOOP AT lt_items ASSIGNING <lfs_worklist_entry> WHERE status IN it_status_select_options.
        APPEND <lfs_worklist_entry> TO rt_items.
      ENDLOOP.
    ELSE.
      rt_items[] = lt_items[].
    ENDIF.
  ...
  METHOD zif_cubi_wrklst~query_review_items.
    ...
    TYPES: matid     TYPE /sapapo/matid,
           createutc TYPE timestamp,
           changeutc TYPE timestamp,
    ...
    FIELD-SYMBOLS: <fs_entry>     TYPE ltyp_entityset,
                   ...
                   <fs_marm>      LIKE LINE OF lt_db_marm.
